...
def index():
    if request.method == "POST":
        ...
        for r in resultados_brutos:
            ...
            if nombre not in totales: totales[nombre] = {"coste": 0, "km": 0}
            totales[nombre]["coste"] += r["coste"]
            totales[nombre]["km"] += r["km"]
        resultados_finales = [{"nombre": n, "coste": round(d["coste"], 2), "km": round(d["km"], 1)} for n, d in totales.items()]
        return render_template("index.html", resultados=sorted(resultados_finales, key=lambda x: x['coste'], reverse=True))
